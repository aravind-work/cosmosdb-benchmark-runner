...
public class AbstractDataGen {
    Logger logger = LoggerFactory.getLogger(AbstractDataGen.class.getSimpleName());

    protected static final int MAX_IDS_TO_FETCH_PER_PARTITION = 1000;
    ...
    protected synchronized boolean recreateCollections(List<String> collectionNames) {
        logger.info("Recreating Collections ...");

        return Observable.from(collectionNames)
                .flatMap(c -> Observable.concat(
                    client.deleteCollection(datagenConfig.tablePrefix + c),
                    client.createCollection(datagenConfig.tablePrefix + c, DataGenConfig.P_KEY, datagenConfig.collectionRusOnCreate,
    ...
    protected synchronized void verifyCollectionsExist(List<String> collectionNames){
        ...
        client.getDatabase()
                .flatMap(db -> {
                    ...
                    return Observable.from(collectionNames)
                            .flatMap(c -> client.getCollection(cosmosConfig.dbName, datagenConfig.tablePrefix + c))
                            .toList(); })
                .subscribeOn(Schedulers.computation())
